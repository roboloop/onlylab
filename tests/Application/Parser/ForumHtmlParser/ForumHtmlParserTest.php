...
{
    /**
     * @dataProvider parseTopicsHeadersData
     */
    public function testParseTopicsHeaders($content, $forumExId, $forumTitle, $topics)
    {
        ...
        $this->assertEquals(count($topics), count($rawTopics));

        for (reset($rawTopics), reset($topics); null !== key($rawTopics); next($rawTopics), next($topics)) {
            $rawTopic = current($rawTopics);
            list($exId, $rawTitle, $size, $exCreatedAt) = current($topics);
    ...
    /**
     * @dataProvider parsePagesData
     */
    public function testParsePages($content, $expectedCurrent, $expectedTotal)
    {
        list($current, $total) = (new ForumHtmlParser)->parsePageState($content);

        $this->assertEquals($expectedCurrent, $current);
        $this->assertEquals($expectedTotal, $total);
    }

    public function parsePagesData()
    {
        return [
            [
    ...
    public function parseTopicsHeadersData()
    {
        return [
            [
                file_get_contents('mdwaylo2.ftmf'),
